...
import { 
  Mic, Video, Award, Clock, TrendingUp, Zap, MessageSquare, Play, Calendar,
  Target, Brain, Activity, Star, ChevronRight, AlertCircle, Search
} from 'lucide-react';
...
const skillData = [
  ...
  { subject: 'Body Language', A: 99, fullMark: 150 },
    ...
    <div className="min-h-screen bg-[#020617] text-white pt-24 pb-12 px-6">
      <div className="max-w-[1600px] mx-auto space-y-8">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          ...
          <div className="flex items-center gap-4 w-full lg:w-auto">
            <div className="relative flex-1 lg:w-80">
              ...
              <input type="text" placeholder="Search sessions..." className="w-full bg-[#0f172a] border border-white/5 rounded-2xl py-3 pl-12 pr-4 text-sm outline-none focus:border-indigo-500 transition-all" />
            </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
          ...
          {/* Progress Chart (Main) */}
          <div className="xl:col-span-8 bg-[#0f172a] border border-white/5 rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden group">
            ...
            <div className="flex justify-between items-center mb-10 relative z-10">
              ...
              <div className="flex gap-2">
                <span className="flex items-center gap-1 text-xs text-indigo-400 font-bold"><div className="w-3 h-3 bg-indigo-500 rounded-full"/> You</span>
                <span className="flex items-center gap-1 text-xs text-slate-500 font-bold"><div className="w-3 h-3 bg-slate-700 rounded-full"/> Avg User</span>
              </div>
            </div>
            <div className="h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={performanceData}>
                  ...
                  <XAxis dataKey="name" stroke="#475569" fontSize={12} axisLine={false} tickLine={false} dy={10} />
          ...
          <div className="xl:col-span-4 space-y-8">
            ...
            <div className="bg-gradient-to-br from-indigo-900/20 to-slate-900 border border-indigo-500/20 rounded-[2.5rem] p-8">
              ...
              <button className="w-full mt-8 py-4 bg-white/5 hover:bg-white/10 rounded-2xl text-xs font-black tracking-widest transition-all">
                VIEW FULL REPORT
              </button>
            </div>
            ...
            <div className="bg-orange-500/10 border border-orange-500/20 rounded-[2.5rem] p-6 flex gap-4">
              <div className="bg-orange-500 text-black w-12 h-12 rounded-2xl flex items-center justify-center shrink-0">
                <AlertCircle size={24} />
              </div>
              <div>
                <h4 className="font-bold text-orange-400">Critical Insight</h4>
                <p className="text-sm text-slate-300 mt-1">You tend to use "Um" frequently in technical answers. Practice pausing instead.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Table */}
          <div className="lg:col-span-2 bg-[#0f172a] border border-white/5 rounded-[2.5rem] p-8 overflow-hidden">
            <h3 className="text-xl font-bold mb-6">Recent Interview Sessions</h3>
            <div className="overflow-x-auto text-white">
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-white/5 text-slate-500 text-xs font-black tracking-widest uppercase">
                ...
                <tbody className="text-sm">
                  ...
                  <SessionRow role="Product Manager" date="Mar 10, 2024" time="45 mins" score="9.2" />
                  <SessionRow role="System Design" date="Mar 08, 2024" time="30 mins" score="7.8" />
                  <SessionRow role="Data Scientist" date="Mar 05, 2024" time="20 mins" score="8.9" />
                </tbody>
              </table>
            </div>
          </div>
          ...
          {/* mini Pie chart */}
          <div className="bg-[#0f172a] border border-white/5 rounded-[2.5rem] p-8 flex flex-col items-center justify-center text-center">
            <h3 className="text-xl font-bold mb-4">Interview Quota</h3>
            ...
            <p className="text-3xl font-black mt-2">40/50</p>
            <p className="text-slate-500 text-sm">Credits remaining for this month</p>
          </div>
...
function StatCard({ icon, label, value, sub, color }) {
  const colors = {
    ...
    green: "text-emerald-500 bg-emerald-500/10 shadow-emerald-500/5",
    yellow: "text-amber-500 bg-amber-500/10 shadow-amber-500/5",
    purple: "text-indigo-500 bg-indigo-500/10 shadow-indigo-500/5",
  };
  return (
    <div className="bg-[#0f172a] border border-white/5 p-8 rounded-[2rem] hover:border-white/20 transition-all shadow-xl group cursor-default">
      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 transition-transform group-hover:scale-110 duration-500 ${colors[color]}`}>
...
function SessionRow({ role, date, time, score }) {
  return (
    <tr className="border-b border-white/5 group hover:bg-white/5 transition-colors cursor-pointer">
      <td className="py-6 px-2 font-bold text-slate-200">{role}</td>
      <td className="py-6 px-2 text-slate-500">{date}</td>
      <td className="py-6 px-2 text-slate-500">{time}</td>
