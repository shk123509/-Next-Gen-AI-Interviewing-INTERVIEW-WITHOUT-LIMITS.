...
export default function UltimateDashboard() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (json?.success && json?.data) {
          setData({
            latestScores: {
              coding: json.data.latestScores?.coding ?? 0,
              aptitude: json.data.latestScores?.aptitude ?? 0,
              communication:
                json.data.latestScores?.communication ?? 0,
            },

            stats: {
              level: json.data.stats?.level ?? "Beginner",
              percentage: json.data.stats?.percentage ?? 0,
            },
          });
        } else {
          setData(defaultData);
  ...
  const handleAnalyzeJob = async () => {
    ...
    try {
      const response = await fetch(
        "https://tool-call-1.onrender.com/chat",
        {
          method: "POST",
      ...
      setAnalysis("❌ Unable to analyze role.");
    } finally {
  ...
  const runPrediction = () => {
    ...
    setTimeout(() => {
      const { coding, aptitude, communication } =
        data.latestScores;
  ...
  if (loading) {
    return (
      <div className="h-screen bg-[#020202] flex flex-col items-center justify-center">
        ...
        <p className="mt-10 text-blue-500 font-black tracking-[0.5em] animate-pulse">
          INITIATING NEXUS.OS
        </p>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans pb-32 pt-28 relative overflow-hidden">
      ...
      <motion.div
        variants={glowVar}
        animate="animate"
        className="fixed -top-[10%] -left-[10%] w-[500px] h-[500px] bg-blue-600/10 blur-[120px] rounded-full -z-10"
      />

      <motion.div
        variants={glowVar}
        animate="animate"
        className="fixed -bottom-[10%] -right-[10%] w-[400px] h-[400px] bg-purple-600/10 blur-[120px] rounded-full -z-10"
      />
      ...
      <motion.main
        ...
        initial="hidden"
        animate="visible"
        className="max-w-7xl mx-auto px-6 space-y-20 relative z-10"
      >
        {/* HEADER */}

        <motion.header
          variants={itemVar}
          className="flex flex-col lg:flex-row justify-between items-end gap-10 border-b border-white/5 pb-16"
        >
          ...
          <div className="flex gap-4">
            <StatCardSmall
              label="RANK"
              val={data.stats.level}
            />

            <StatCardSmall
              label="EFFICIENCY"
              val={`${data.stats.percentage}%`}
              blue
            />
          </div>
        </motion.header>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <StatBox
            icon={<Brain />}
            label="LOGIC"
            val={data.latestScores.aptitude}
            color="purple"
          />
          ...
          <StatBox
            icon={<MessageSquare />}
            label="VERBAL"
            val={data.latestScores.communication}
            color="emerald"
          />
        </section>
            ...
            <div className="bg-[#0a0a0a] border border-white/5 rounded-[3rem] p-12 flex flex-col items-center justify-center min-h-[400px]">
              <AnimatePresence mode="wait">
                ...
                !isPredicting ? (
                  ...
                  >
                    <div
                      className={`text-9xl font-black italic tracking-tighter ${predictionResult.color}`}
                    >
                  ...
                  <div className="opacity-10 font-black tracking-[0.5em] text-xs">
                    AWAITING INPUT
                  </div>
                )}
              </AnimatePresence>
            ...
            <div className="relative z-10 space-y-12">
              <div className="relative flex items-center max-w-2xl mx-auto">
                ...
                <button
                  ...
                  className="absolute right-4 bg-indigo-600 px-10 py-5 rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest"
                >
                  {isAnalyzing ? (
...
function StatBox({
  ...
  return (
    ...
    >
      ...
      <div className="p-5 rounded-2xl bg-white/[0.03] border border-white/5 w-fit mb-12 group-hover:rotate-12 transition-transform duration-500">
        {icon}
      </div>
      ...
      <div className="flex items-baseline gap-2">
        <span className="text-8xl font-black italic tracking-tighter text-white leading-none">
          {val}
        </span>
...
function StatCardSmall({
  ...
  return (
    <div
      className={`px-10 py-6 rounded-[2rem] border border-white/5 ${
        blue ? "bg-blue-600" : "bg-[#0a0a0a]"
      }`}
    >
      <p
        className={`text-[8px] font-black uppercase mb-1 tracking-widest ${
          blue ? "text-white/50" : "text-blue-500"
        }`}
      >
        {label}
      </p>
